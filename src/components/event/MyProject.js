    ...
    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <h6 className="fs-0 mb-1 " style={{ color: 'red' }}>
              ...
              <Link
                to={{
                  ...
                  state: { projectNo: project.projectNo, projectTitle: project.projectTitle, members: project.members }
                }}
              >
          ...
          <Button
            style={{ backgroundColor: 'white', border: '0px', marginTop: '-5px' }}
            onClick={() => {
              btnclick();
            }}
          >
            <FontAwesomeIcon style={{ color: 'gray' }} icon={faCog} />
          </Button>
        </div>
        ...
        <div className="mb-1">
          ...
          {projectmembers.map(member => (
            <Member key={member.no} member={member} />
          ))}
        </div>
      </div>

      {/* DetailModal 모달창 생성 */}
      {projectDetailModal == false ? null : (
        <Modal
        ...
        >
          <div className="modal-content p-3">
            <headers className=" d-flex flex-between-center border-bottom-0">
              ...
              <button
                style={{ backgroundColor: 'white', border: '0px', marginTop: '-10px' }}
                onClick={() => modalFalse()}
              >
                {' '}
            ...
            <body className="p-2">
              <FormGroup>
                ...
                <Input
                  ...
                  required
                  onChange={({ target }) => modalhandleChange(target)}
                />
              </FormGroup>
              ...
              <FormGroup>
                ...
                <Input
                  name="projectUpdateDesc"
                  id="eventDesc"
                  value={projectUpdateDesc}
                  required
                  onChange={({ target }) => modalhandleChange(target)}
                />
              </FormGroup>
              ...
              <FormGroup>
                ...
                <br />
                <button
                  style={{ backgroundColor: '#EDF2F9', border: '0px' }}
                  onClick={() => {
                    memberSearchandInput();
                  }}
                  className="mr-3 "
                >
                  <FontAwesomeIcon style={{ color: '#27BCFD', fontSize: '25px' }} icon={faPlus} />
                </button>

                {memberInputOpen == false ? null : (
                  <input
                    id="member"
                    name="meber"
                    placeholder="멤버 찾기"
                ...
                {searchInputOpen == false ? null : !searchMembers ? null : (
                  //멤버 리스트 출력
                  <div
                    className="mt-2"
                    style={{
                  ...
                  >
                    {searchMembers
                      .filter(
                        searchMember =>
                          searchMember.userName.indexOf(keyword) !== -1 ||
                          searchMember.userEmail.indexOf(keyword) !== -1
                      )
                      .map(searchMember => (
                        <div
                          ...
                          onClick={() => {
                            memberselect(
                              ...
                              searchMember.userEmail,
                              searchMember.userPhoto
                            );
                          }}
                          key={searchMember.userNo}
                        >
                          <span style={{ fontWeight: 'bold', color: 'rgb(94,110,130)' }}>{searchMember.userName}</span>{' '}
                          <span>{searchMember.userEmail}</span>
                        </div>
                      ))}
                  </div>
                )}
              ...
              <FormGroup>
                ...
                {!m ? null : (
                  <div style={{ marginLeft: '20px' }}>
                    {m.map(mlist => (
                      <div>
                        {mlist.userName} {mlist.userEmail}{' '}
                      </div>
                    ))}
                  </div>
                )}
